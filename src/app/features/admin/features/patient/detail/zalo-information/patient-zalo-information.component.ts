...
@Component({
  selector: 'app-patient-zalo-information',
  templateUrl: './patient-zalo-information.component.html',
  standalone: true,
  imports: [FontAwesomeModule, CommonModule, AdminModalConfirmComponent, FormsModule]
})
export class patientZaloInformationComponent implements OnInit, OnDestroy {
  faUser = faUser;
  faPen = faPen;
  ...
  ngOnInit(): void {
    this.route.parent?.paramMap.pipe(takeUntil(this.destroy$)).subscribe((params) => {
      this.userId = params.get('userId'); // zaloUid
      if (this.userId) {
        this.fetchUser();
      }
    });
