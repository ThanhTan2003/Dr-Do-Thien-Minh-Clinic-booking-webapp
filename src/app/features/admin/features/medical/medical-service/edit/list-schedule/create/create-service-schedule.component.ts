...
@Component({
  selector: 'app-create-service-schedule',
  templateUrl: './create-service-schedule.component.html',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class CreateServiceScheduleComponent implements OnInit {
  ...
  onConfirmSubmit(): void {
    const requests: ServiceScheduleRequest[] = this.scheduleItems.map(item => ({
    ...
    this.serviceScheduleService.createOrUpdateBatch(requests).subscribe({
      next: () => {
